...
/*Pitch bias calculate*/
void calculate_pitch_bias() {
	// Number of samples to average
	int num_samples = 1000;
	float pitchSum = 0;

	for (int i = 0; i < num_samples; i++) {
		mpu_data reading = mpu6050_data();
		pitchSum += atan(reading.ax / sqrt(reading.ay*reading.ay + reading.az*reading.az)) * 180 / M_PI;
	}
	pitchBias = pitchSum / num_samples;
}
